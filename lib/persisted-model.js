  ...
  PersistedModel.subscribe = function(options, cb) {
    ...
    function createChangeHandler(type, optimistic) {
      return function(ctx, next) {
        // note: this leaks the ref and writeable variables
        // but not the entire change stream (with its huge buffer)

        // dont reference the closure variable directly
        var changes = ref.changes;

        // since it might have been deleted (via destroy())
        if(!changes) {
          return next();
        }
